...
public class AuthenticationWebFilter implements WebFilter {
    ...
    public static final Map<String, Category> TOKEN_MAP = Map.of(
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        String token = exchange.getRequest()
                .getHeaders()
                .getFirst("auth-token");
        if(Objects.nonNull(token) && TOKEN_MAP.containsKey(token)){
            exchange.getAttributes().put("category", TOKEN_MAP.get(token));
